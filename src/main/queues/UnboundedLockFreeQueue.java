...
import edu.vt.ece.hw6.queues.EmptyException;
 ...
 */
public class UnboundedLockFreeQueue<T> {
  ...
  private static synchronized <T> boolean multiCompareAndSet(
      AtomicReference<T>[] target,
      T[] expect,
      T[] update) {
    for (int i = 0; i < target.length; i++) {
      if (!target[i].compareAndSet(expect[i], update[i])) {
        return false;
      }
    }
    return true;
